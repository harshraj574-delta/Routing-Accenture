...
async function checkRouteDeviation(route, facility, profile) {
  ...
  let rules;
  let effectiveRuleKey = "UNKNOWN";

  if (ruleKeys.length === 0) {
    console.warn(
      `[checkRouteDeviation] Route ${route.routeNumber}: profile.routeDeviationRules is empty. No rules to apply. Returning true (lenient).`
    );
    return true;
  }
  ...
  if (ruleKeys.length > 1) {
    console.warn(
      `[checkRouteDeviation] Route ${
        route.routeNumber
      }: profile.routeDeviationRules contains multiple keys (${ruleKeys.join(
        ", "
      )}). Expected a single key (e.g., "CDC", "DDC"). Using rules from the first key: "${
        ruleKeys[0]
      }".`
    );
    // Fallback: use the first key if multiple are unexpectedly present
    effectiveRuleKey = ruleKeys[0];
    rules = profile.routeDeviationRules[effectiveRuleKey];
  } else {
    // Exactly one key, as expected
    effectiveRuleKey = ruleKeys[0];
    rules = profile.routeDeviationRules[effectiveRuleKey];
    console.log(
      `[checkRouteDeviation] Using deviation rules for key: "${effectiveRuleKey}" from profile.routeDeviationRules.`
    );
  }
  ...
  if (!rules || !Array.isArray(rules) || rules.length === 0) {
    console.warn(
      `[checkRouteDeviation] Route ${
        route.routeNumber
      }: No valid deviation rules found for key "${effectiveRuleKey}" or rules array is empty. Returning true (lenient).`
    );
    return true;
  }

  console.log(
    `[checkRouteDeviation] Found ${
      rules.length
    } rules for key "${effectiveRuleKey}":`,
    rules
      .map((r) => `${r.minDistKm}-${r.maxDistKm}km (max: ${r.maxTotalOneWayKm}km)`)
      .join(", ")
  );
  ...
  console.log(
    `[checkRouteDeviation] Route ${
      route.routeNumber
    }: Max OSRM distance from Facility to an employee: ${maxOsrmDistToEmployeeKm.toFixed(
      3
    )} km (exact: ${maxOsrmDistToEmployeeKm})`
  );
  ...
  if (!applicableRule || applicableRule.maxTotalOneWayKm == null) {
    console.error(
      `[checkRouteDeviation] Could not determine applicable rule for Route ${
        route.routeNumber
      }. Max emp distance: ${maxOsrmDistToEmployeeKm.toFixed(
        3
      )}km. Rules attempted from key "${effectiveRuleKey}". Returning FALSE.`
    );
    return false;
  }

  console.log(
    `[checkRouteDeviation] Route ${route.routeNumber}: Applicable rule (from key "${effectiveRuleKey}"): ${applicableRule.minDistKm}-${applicableRule.maxDistKm}km, maxTotalOneWayKm: ${applicableRule.maxTotalOneWayKm}`
  );
  ...
  console.log(
    `[checkRouteDeviation] Route ${
      route.routeNumber
    }: Actual OSRM for ENTIRE ROUTE: ${relevantRouteDistanceKm.toFixed(
      3
    )} km. Max allowed by rule: ${maxAllowedKmForRule} km.`
  );
  ...
  if (relevantRouteDistanceKm > maxAllowedKmForRule) {
    console.warn(
      `[checkRouteDeviation] Route ${
        route.routeNumber
      }: ROUTE DEVIATION EXCEEDED. Allowed: ${maxAllowedKmForRule}km, Actual route: ${relevantRouteDistanceKm.toFixed(
        3
      )}km. Returning false.`
    );
    return false;
  ...
  console.log(
    `[checkRouteDeviation] Route ${route.routeNumber}: Deviation OK. Returning true.`
  );
  return true;
}
...
async function processEmployeeBatch(
  ...
  mainLoop: while (globalRemainingEmployees.length > 0) {
    ...
    const deviationOkay = checkRouteDeviation(currentRoute, facility, profile);
    if (!deviationOkay) {
    ...
    while (
      ...
      const tentativeDeviationOkay = checkRouteDeviation(tentativeRoute, facility, profile);
