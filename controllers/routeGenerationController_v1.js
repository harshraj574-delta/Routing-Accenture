/**
 * Route Generation Controller v1 (Legacy)
 * Uses the original routing algorithm from routeGenerationService_v1.js
 */

const { v4: uuidv4 } = require('uuid');
const routeGenerationService = require('../services/routeGenerationService_v1');
const { routeRequestSchema, recalculateRouteRequestSchema, etaRequestSchema } = require('../validators/routeGenerationValidator');

const routeGenerationControllerV1 = {
    getEta: async (req, res) => {
        try {
            ...
            console.error('[v1] Error calculating ETA:', error);
    ...
    generateRoutes: async (req, res) => {
        ...
        try {
            ...
            console.log(`[v1] Generating routes for ${employees.length} employees, date: ${date}, shift: ${shiftTime}`);

            const uuid = uuidv4();

            if (zones && Array.isArray(zones) && zones.length > 0) {
                console.log(`[v1] Using ${zones.length} zones provided in request`);
            } else {
                console.log('[v1] No zones provided in request, will use zones from backend data file');
            }
            ...
            if (transaction && !transaction.finished) {
                try {
                    ...
                    console.log('[v1] Transaction rolled back due to error.');
                } catch (rollbackError) {
                    console.error('[v1] Error rolling back transaction:', rollbackError);
                }
            }
            console.error('[v1] Error generating routes:', error);
    ...
    recalculateRoute: async (req, res) => {
        try {
            ...
            return res.status(200).json({ recalculatedRoutes, apiVersion: 'v1' });
        } catch (error) {
            console.error('[v1] Error recalculating route:', error);
        ...
        try {
            ...
            for (const route of routes) {
                ...
                const reoptResult = await routeGenerationService.reoptimizeRouteWithVRP({
                    employees, facility, tripType,
                    vehicleCapacity: vehicleCapacity || employees.length,
                    pickupTimePerEmployee, city, shiftTime
                });

                if (!reoptResult.reOptimized || reoptResult.error) {
                    console.warn(`[v1] Reoptimize failed for route ${routeId}, falling back to original order`);
                    const employeeCoordinates = employees.map(emp => [emp.geoY, emp.geoX]);
                    const facilityCoordinates = [facility.geoY, facility.geoX];
                    const routeCoordinates = isDropoff ? [facilityCoordinates, ...employeeCoordinates] : [...employeeCoordinates, facilityCoordinates];
            ...
            return res.status(200).json({ recalculatedRoutes: reoptimizedRoutes, apiVersion: 'v1' });
        } catch (error) {
            console.error('[v1] Error reoptimizing route:', error);
            res.status(500).json({ error: 'Failed to reoptimize route', details: error.message });
        }
    }
};

module.exports = routeGenerationControllerV1;
